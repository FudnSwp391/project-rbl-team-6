...
const SchedulePage = () => {
  ...
  useEffect(() => {
    fetchSchedule();
  }, [searchQuery, statusFilter, subjectFilter, tutorFilter]);
  ...
  const markCompleted = async (sessionId) => {
    try {
      ...
      if (json.success) {
        ...
        alert(json.error || 'Error marking as completed');
      }
    } catch (err) {
      alert('Network error');
    }
  ...
  const getSessionStyle = (status) => {
    ...
    if (status === 'upcoming') return { bgColor: 'bg-tertiary-container/10', borderColor: 'border-tertiary/20', labelBg: 'bg-tertiary-container', labelColor: 'text-tertiary' };
    ...
    if (status === 'cancelled') return { bgColor: 'bg-error-container/10', borderColor: 'border-error/20', labelBg: 'bg-error', labelColor: 'text-on-error' };
    ...
    <div className="flex flex-col gap-lg pb-4">
      ...
      <section className="flex justify-between items-end">
        ...
        <div className="flex items-center gap-4">
          ...
          <button type="button" className="bg-primary text-on-primary px-5 py-2.5 rounded-lg font-label-md text-label-md flex items-center gap-2 hover:bg-primary/90 transition-colors shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)] h-12" onClick={() => alert('Book session modal not fully implemented')}>
      ...
      {loading ? (
        ...
        <>
          {/* Stats Row */}
          <section className="bg-primary-container/5 border border-primary/20 rounded-xl p-5 flex items-center shadow-sm gap-6">
            <div className="flex items-center gap-4 shrink-0">
              <div className="w-14 h-14 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                <span className="material-symbols-outlined text-3xl">military_tech</span>
              </div>
              <div>
                <h3 className="text-title-lg font-bold text-on-surface">This Week's Progress</h3>
                <p className="text-label-md text-on-surface-variant">{data.summary.weekly_completed} / {data.summary.weekly_total} sessions completed</p>
              </div>
            </div>
            
            <div className="flex-1 px-4 border-l border-r border-outline-variant/30 max-w-[300px]">
              <div className="flex justify-between mb-1">
                <span className="text-label-sm font-bold text-primary">
                  {data.summary.weekly_total > 0 ? Math.round((data.summary.weekly_completed / data.summary.weekly_total) * 100) : 0}% Complete
                </span>
                <span className="text-[11px] text-on-surface-variant">Goal: {data.summary.weekly_goal_hours}h</span>
              </div>
            ...
            <div className="flex flex-wrap gap-2 flex-1 justify-end">
              <div className="bg-surface border border-outline-variant/50 rounded-full px-3 py-1.5 flex items-center gap-1.5 shadow-sm">
                ...
                <span className="text-label-sm font-bold text-on-surface">{data.summary.completed} Classes</span>
              </div>
              <div className="bg-surface border border-outline-variant/50 rounded-full px-3 py-1.5 flex items-center gap-1.5 shadow-sm">
                <span className="material-symbols-outlined text-[14px] text-tertiary">timer</span>
                <span className="text-label-sm font-bold text-on-surface">{data.summary.learning_hours}h Studied</span>
              </div>
              <div className="bg-surface border border-outline-variant/50 rounded-full px-3 py-1.5 flex items-center gap-1.5 shadow-sm">
                <span className="material-symbols-outlined text-[14px] text-error">local_fire_department</span>
                <span className="text-label-sm font-bold text-on-surface">{data.summary.streak_days} Streak</span>
              </div>
              <div className="bg-surface border border-outline-variant/50 rounded-full px-3 py-1.5 flex items-center gap-1.5 shadow-sm">
          ...
          <section className="grid grid-cols-4 gap-4">
            <div className="bg-surface rounded-lg p-4 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05),_0_2px_4px_-2px_rgba(0,0,0,0.05)] flex items-center gap-4">
              ...
              <div>
                <p className="text-label-sm font-label-sm text-on-surface-variant">Total Classes</p>
            ...
            <div className="bg-surface rounded-lg p-4 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05),_0_2px_4px_-2px_rgba(0,0,0,0.05)] flex items-center gap-4">
              ...
              <div>
                <p className="text-label-sm font-label-sm text-on-surface-variant">Completed</p>
            ...
            <div className="bg-surface rounded-lg p-4 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05),_0_2px_4px_-2px_rgba(0,0,0,0.05)] flex items-center gap-4">
              ...
              <div>
                <p className="text-label-sm font-label-sm text-on-surface-variant">Upcoming</p>
            ...
            <div className="bg-surface rounded-lg p-4 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05),_0_2px_4px_-2px_rgba(0,0,0,0.05)] flex items-center gap-4">
              ...
              <div>
                <p className="text-label-sm font-label-sm text-on-surface-variant">Learning Hours</p>
          ...
          <section className="flex gap-4">
            ...
            <select 
              className="px-4 py-2.5 bg-surface rounded-lg border border-outline-variant focus:border-primary focus:ring-2 focus:ring-primary/20 text-body-md font-body-md text-on-surface h-12 w-40"
              value={statusFilter}
          ...
          <div className="flex gap-gutter items-stretch min-h-[calc(100vh-450px)] mb-4">
            {/* Weekly Schedule Board */}
            <section className="bg-surface rounded-xl shadow-sm border border-surface-variant w-[70%] flex flex-col h-full">
              {/* Days Header */}
              <div className="grid grid-cols-7 border-b border-surface-variant bg-surface-container-lowest rounded-t-xl shrink-0">
                {weekDates.map((wd, index) => (
                  <div key={index} className={`py-3 text-center border-r border-surface-variant ${wd.isToday ? 'bg-primary/10' : ''}`}>
                    ...
                    {wd.isToday && <div className="text-[10px] font-bold text-primary mt-1">Today</div>}
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-7 bg-surface-container-lowest rounded-b-xl flex-1">
                {weekDates.map((wd, index) => {
                  ...
                  return (
                    <div key={index} className={`border-r border-surface-variant p-2 flex flex-col gap-2 ${wd.isToday ? 'bg-primary/5' : 'bg-surface'} h-full`}>
                      {daySessions.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-8 text-on-surface-variant opacity-50 flex-1">
                          ...
                          <span className="text-[11px] font-medium">No class</span>
                        </div>
                      ) : (
                        <>
                            ...
                            return (
                              <div key={session.id} className={`w-full ${style.bgColor} border ${style.borderColor} rounded-lg p-2.5 hover:shadow-md transition-shadow cursor-pointer shadow-sm`}>
                                ...
                                <div className="flex items-center gap-1.5 mt-auto pt-1 border-t border-surface-variant/50">
                                  <div className="w-4 h-4 rounded-full bg-surface-variant flex items-center justify-center text-[8px] font-bold">{session.tutor_name.charAt(0)}</div>
            ...
            <div className="w-[30%] flex flex-col">
              ...
              <section className="bg-surface rounded-xl p-5 shadow-sm border border-surface-variant flex flex-col h-full">
                <div className="flex justify-between items-center mb-4 shrink-0">
                  ...
                  <button type="button" className="text-primary hover:underline text-label-sm font-label-sm font-bold">View all</button>
                </div>
                
                <div className="mb-4 p-3 bg-primary/5 rounded-lg border border-primary/10 shrink-0">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-label-sm font-bold text-on-surface">Today's Classes</span>
                ...
                <div className="flex flex-col gap-3 flex-1 overflow-visible">
                  {data.up_next.length === 0 ? (
                    <p className="text-center text-on-surface-variant text-label-md py-4">No upcoming classes</p>
                  ) : (
                      ...
                      <div key={session.id} className={`${index === 0 ? 'bg-primary-container/5 border border-primary/20' : 'bg-surface border border-outline-variant'} rounded-lg p-4 hover:shadow-[0_10px_15px_-3px_rgba(0,0,0,0.08)] transition-all`}>
                        ...
                        {index !== 0 && session.status !== 'completed' && (
                          <button 
                            ...
                            className="mt-2 text-primary font-label-sm text-[12px] underline"
                          >
                            Mark Completed (Dev)
                          </button>
                        )}
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-surface-variant shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-title-lg font-bold text-on-surface">Classes on {formatDayTitle(selectedDay.date)}</h3>
                  <p className="text-label-sm text-on-surface-variant">{selectedDay.sessions.length} session{selectedDay.sessions.length !== 1 ? 's' : ''} scheduled</p>
                </div>
              </div>
              <button
                type="button"
